#!/bin/bash
# ==============================================================================
#  LAMP + WordPress Server - VARIANTA FINALA (consolidata)
#  "Final boss" - pregatire completa pentru Ubuntu Server clean install
# ------------------------------------------------------------------------------
#  STACK:
#    - Apache2 + ModSecurity (WAF, OWASP CRS, mod blocare) + mod_evasive (anti-DoS)
#    - PHP + TOATE dependintele necesare WordPress (mysql,curl,xml,mbstring,zip,
#      gd,imagick,intl,bcmath,soap,opcache) + hardening php.ini
#    - MariaDB -> root FARA parola, accesibil EXCLUSIV prin `sudo mariadb`
#      (autentificare unix_socket). Orice comanda admin DB doar din contul tau,
#      prin sudo. Userii per-site se creeaza automat de 'add-wp-site'.
#
#  SECURITATE (gandita ca un cybersecurity manager):
#    - SSH pe PORT CUSTOM ales manual; UFW + Fail2Ban configurate pe acel port
#    - SSH hardening: root off, cripto moderna, ssh.socket gestionat (Ubuntu 24.04)
#    - UFW: deny in/out, web public (80/443), admin (SSH+Webmin) DOAR din LAN
#      + protectie anti auto-lockout (regulile inainte de enable + confirmare)
#    - Fail2Ban: 13 jail-uri (SSH, Apache, ModSecurity, WordPress login/xmlrpc,
#      scanere, fisiere sensibile, badbots, shellshock, Webmin, recidive).
#      Backend: 'auto' (inotify pe fisiere) pentru jail-urile Apache/WordPress,
#      'systemd' DOAR pentru sshd si webmin-auth, care chiar logheaza in journal.
#    - Hardening OS: auditd, AppArmor enforce, pwquality, faillock, sysctl kernel,
#      blacklist module, /dev/shm securizat, core dumps off, cron restrictionat
#    - Webmin instalat din pachet .deb descarcat manual, restrictionat la LAN
#    - Actualizari automate de securitate (+ reboot controlat la 04:00)
#
#  Compatibil: Ubuntu Server 22.04 / 24.04 LTS
#  Rulare:     sudo bash final_boss.sh
#  Verificat:  shellcheck -S style (0 erori, 0 avertismente) + bash -n
#              terminatii de linie LF (NU CRLF - un script bash cu CRLF nu ruleaza)
# ==============================================================================
...
read -r -p "  -> Confirma subreteaua LAN (Enter = ${DETECTED_SUBNET}): " LAN_SUBNET
...
echo
echo "  Portul standard SSH (22) este primul lucru testat de orice scaner."
echo "  Alegerea unui port custom (ex: 2222, 4822, 49152) elimina 99% din"
echo "  traficul de boti automate inainte ca Fail2Ban sa intervina."
echo
while true; do
...
if [[ "$SSH_PORT" == "22" ]]; then
    warn "Ai ales portul standard 22. Recomandam un port custom pentru securitate sporita."
else
    info "Port SSH custom: $SSH_PORT"
fi
...
if apache2ctl configtest 2>/dev/null; then
    ...
    warn "Configtest Apache a raportat o problema. Verific detaliile..."
    apache2ctl configtest || true
    error "Configuratia Apache este invalida. Corecteaza inainte de a continua."
fi
...
# ATENTIE - NU pune aici 'backend = systemd'.
# fail2ban IGNORA complet 'logpath' cand backend-ul e systemd. In jailreader.py:
#     if opt == "logpath":
#         if backend.startswith("systemd"): continue
# Cu un backend systemd global, toate jail-urile Apache/WordPress de mai jos ar
# aparea "verzi" in 'fail2ban-client status', dar ar asculta pe journald - unde
# logurile Apache nu ajung NICIODATA. Rezultat: zero banari, fals sentiment de
# siguranta. Backend-ul implicit ('auto' = inotify pe fisiere) e cel corect aici;
# systemd se pune DOAR pe sshd, care chiar logheaza in journal.
# Verificare: sudo fail2ban-client get wordpress-login logpath  (nu trebuie gol)
...
# maxretry/findtime relaxate fata de 10/60: un plugin sau o tema cu un fisier
# lipsa genereaza 404-uri la FIECARE vizitator, iar 10 intr-un minut se atinge
# usor din navigare normala => vizitatori reali banati 7 zile. Un scanner real
# (WPScan/Nikto) face sute de cereri pe minut, deci 15/120 il prinde oricum.
[wordpress-scanner]
enabled  = true
port     = http,https
...
# ── Webmin - brute-force pe panoul de administrare ───────────────────────────
# Filtrul oficial 'webmin-auth' citeste din SYSLOG, nu din /var/webmin/miniserv.log
# (acela e access log in format Apache si NU contine mesajele de login esuat).
# De aceea punem 'syslog=1' in miniserv.conf la sectiunea Webmin.
[webmin-auth]
enabled  = true
port     = 10000
filter   = webmin-auth
...
# Asiguram ca exista access.log/error.log default (jail-urile WordPress folosesc
# globul *access.log; pe instalare proaspata exista doar access.log default).
touch /var/log/apache2/access.log /var/log/apache2/error.log 2>/dev/null || true

# Restart tolerant - daca un jail are o problema, nu abortam tot scriptul
if systemctl restart fail2ban; then
    ...
    warn "Continui instalarea (Fail2Ban poate fi reparat ulterior)."
fi
...
# IMPORTANT: ordinea corecta este:
#   1. reset + setare politici default (fara enable inca)
#   2. adauga TOATE regulile, inclusiv SSH pe portul nou
#   3. enable abia la final
# Daca activam UFW inainte de a adauga regula SSH, conexiunea curenta e taiata
# si ramanem blocati afara (auto-lockout).

ufw --force reset

# Politici default - inca nu activam
...
# Admin DOAR din LAN - adaugat INAINTE de enable
# SSH pe portul custom: 'limit' include si permisiunea + rate limiting (max 6/30s)
...
# Verifica daca scriptul ruleaza printr-o sesiune SSH activa
# si avertizeaza utilizatorul inainte de activarea UFW
if [[ -n "${SSH_CLIENT:-}" ]] || [[ -n "${SSH_TTY:-}" ]]; then
    echo
    warn "Scriptul ruleaza printr-o sesiune SSH activa."
    warn "UFW va fi activat acum cu regula SSH pe portul ${SSH_PORT} deja in loc."
    warn "Conexiunea curenta poate fi intrerupta daca portul sau subreteaua LAN"
    warn "nu sunt configurate corect. Regula adaugata:"
    warn "  -> SSH permis din ${LAN_SUBNET} pe portul ${SSH_PORT}"
    echo
...
# Activare UFW abia dupa ce TOATE regulile sunt in loc, inclusiv SSH
# Astfel conexiunea SSH curenta nu este intrerupta
ufw --force enable
info "UFW activat: web public (80/443); SSH port ${SSH_PORT} + Webmin doar din $LAN_SUBNET."
...
if [[ -f /etc/webmin/miniserv.conf ]]; then
    # Stergem intai orice 'allow=' existent, apoi scriem al nostru. Varianta
    # 'grep -q || echo' ar sari peste restrictionare daca ar exista deja o linie
    # allow= (permisiva), lasand Webmin-ul deschis catre toata lumea.
    ...
    # Esecurile de login trebuie sa ajunga in syslog -> acolo citeste jail-ul
    # fail2ban 'webmin-auth'. Fara asta, panoul de admin ramane fara protectie
    # anti brute-force.
...
# Heredoc fara ghilimele la EOF -> permite expansiunea variabilei $SSH_PORT
...
EOF
# Ubuntu 24.04: SSH foloseste socket activation (ssh.socket), iar directiva 'Port'
# din sshd_config este IGNORATA. Dezactivam ssh.socket si fortam ssh.service clasic,
# altfel portul custom ${SSH_PORT} nu s-ar aplica niciodata.
if systemctl list-unit-files | grep -q '^ssh.socket'; then
    systemctl disable --now ssh.socket 2>/dev/null || true
    # Curatam orice override de port din socket
...
# Verificare ca SSH chiar asculta pe portul nou
sleep 1
if ss -tlnp 2>/dev/null | grep -q ":${SSH_PORT} "; then
    ...
    warn "Verifica manual: sudo ss -tlnp | grep ssh"
fi
info "SSH hardened pe portul ${SSH_PORT}. Parola ramane activa (pune chei SSH si dezactiveaz-o ulterior)."
...
# Pe Ubuntu modern mountpoint-ul real e /dev/shm - /run/shm a disparut prin 14.04.
# Montarea lui /run/shm nu hardeneste nimic si lasa o unitate systemd esuata la boot.
...
# ── Helper add-static-site (site-uri statice HTML/CSS/JS, fara DB/PHP) ────────
cat > /usr/local/bin/add-static-site << 'STATICSCRIPT'
#!/bin/bash
# Creeaza un site STATIC (HTML/CSS/JS) - fara WordPress, fara baza de date.
# Utilizare: sudo add-static-site domeniu.ro
set -euo pipefail
[[ "$EUID" -ne 0 ]] && { echo "Ruleaza cu sudo."; exit 1; }

DOMAIN="${1:?Lipseste domeniu. Ex: add-static-site domeniu.ro}"
WEB_ROOT="/var/www/${DOMAIN}"

echo "[+] Director web: $WEB_ROOT"
mkdir -p "$WEB_ROOT"

# Creeaza un index.html minimal doar daca nu exista deja unul
...
<head>
    ...
    <title>Site activ</title>
</head>
<body>
    <h1>Site static functional</h1>
    <p>Inlocuieste acest fisier cu continutul tau.</p>
</body>
</html>
HTML
    echo "[+] index.html demonstrativ creat (il poti inlocui)."
fi
...
# VirtualHost - fara reguli WordPress, doar servire statica
...
echo ""
echo "=================================================="
echo "  Site static adaugat: http://${DOMAIN}"
echo "  Web root:            ${WEB_ROOT}"
echo ""
echo "  Incarca fisierele tale (index.html, css, js, imagini) in:"
echo "    ${WEB_ROOT}/"
echo ""
echo "  Dupa upload, seteaza permisiunile cu:"
...
echo "=================================================="
STATICSCRIPT
chmod +x /usr/local/bin/add-static-site
info "Helper 'add-static-site' instalat in /usr/local/bin/."
...
echo
echo "  === PASI URMATORI ==="
echo "  1a. Site WordPress:  sudo add-wp-site domeniu.ro nume_db user_db parola_db"
echo "  1b. Site static:     sudo add-static-site domeniu.ro"
...
echo "  4. Conectare SSH de acum inainte:"
echo "       ssh -p ${SSH_PORT} ${ADMIN_OS_USER}@${SERVER_IP}"
echo "  5. (Recomandat) Pune chei SSH, apoi dezactiveaza parola:"
echo "       ssh-copy-id -p ${SSH_PORT} ${ADMIN_OS_USER}@${SERVER_IP}   (de pe masina ta)"
