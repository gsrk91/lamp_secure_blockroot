...
#    - Fail2Ban: 11 jail-uri (SSH, Apache, ModSecurity, WordPress login/xmlrpc,
#      scanere, fisiere sensibile, badbots, shellshock, recidive) + backend journald
#    - Hardening OS: auditd, AppArmor enforce, pwquality, faillock, sysctl kernel,
#      blacklist module, /run/shm securizat, core dumps off, cron restrictionat
...
#  Rulare:     sudo bash setup_lamp_wordpress_FINAL.sh
#  Verificat:  shellcheck (0 erori, 0 avertismente) + bash -n
# ==============================================================================
...
echo
while true; do
    read -p "  -> Port custom SSH (1024-65535, Enter = 22 standard): " SSH_PORT
...
# Ubuntu 24.04 logheaza prin systemd-journald (nu mereu /var/log/auth.log).
# backend=systemd asigura ca jail-urile citesc din journal.
backend         = systemd
...
if [[ -n "${SSH_CLIENT:-}" ]] || [[ -n "${SSH_TTY:-}" ]]; then
    ...
    echo
    read -p "  Continua cu activarea UFW? (da/nu): " UFW_CONFIRM
